        ...
        <div className="relative w-full h-full group/carousel">
            ...
            {images.length > 1 && (
                <>
                    <button
                        onClick={prev}
                        className="absolute left-1.5 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-200"
                    ...
                    <button
                        onClick={next}
                        className="absolute right-1.5 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-200"
                    ...
                    <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 flex gap-1">
                        {images.map((_, i) => (
                            <button
                                ...
                                className={`w-1.5 h-1.5 rounded-full transition-colors duration-200 ${i === current ? "bg-white" : "bg-white/40"
                                    }`}
...
export default function ProjectCard({ project, index }: ProjectCardProps) {
    ...
    const images = project.previewImages ?? (project.previewImage ? [project.previewImage] : null);

    return (
        <article className="card flex flex-col overflow-hidden group" aria-label={`Progetto: ${project.title}`}>
            {/* Preview immagine, carousel o placeholder terminale */}
            <div className="mb-6 flex h-44 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-950/60 overflow-hidden">
                {images ? (
                    <ImageCarousel images={images} alt={project.imageAlt} />
                ) : (
            ...
            {/* Link GitHub — nascosto se showGithub è false */}
